...
import { totalIncome, totalExpenses, totalProfitLoss, totalMonthsRecorded } from '@/lib/mock-data/finance'
...
function buildReports() {
  ...
  const months = totalMonthsRecorded()
  ...
  return [
    ...
    { title: 'Revenue', summary: `${formatCurrency(income)} recorded income (${months} months, P&L sheet)` },
    { title: 'Expenses', summary: `${formatCurrency(expenses)} recorded expenses (${months} months, P&L sheet)` },
    ...
    { title: 'Profitability', summary: `Net ${profitLoss >= 0 ? 'profit' : 'loss'} of ${formatCurrency(Math.abs(profitLoss))} over ${months} months - top crop by income: ${topCrop?.cropName}` },
  ]
}
